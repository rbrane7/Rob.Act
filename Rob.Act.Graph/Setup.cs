...
namespace Rob.Act.Analyze
{
	...
	public class State
	{
		...
		internal Main Context { get => context ; set { context = value.Set(c=>{ c.PropertyChanged += Change ; Path = Main.Setup.StatePath ; }) ; Saver?.Dispose() ; Saver = new System.Threading.Timer(Save,null,Main.Setup.SavePeriod,Main.Setup.SavePeriod) ; Load() ; } } Main context ;
