...
namespace Rob.Act.Gpx
{
	...
	public partial class gpxType
	{
		...
		public static implicit operator Path( gpxType way ) => way.Get( w => new Path(w.metadata.time,w.Iterator,Translation.Kind,(Axis.Beat,60)) { Object = w.trk?.Select(t=>t.type).Distinct().Stringy(',') , Action = w.trk?.Select(t=>t.name.Action()).Stringy(',') , Subject = w.trk?.Select(t=>t.link.At(0)?.href.Subject()).Distinct().Stringy(',') , Locus = w.trk?.Select(t=>t.name.Locus()).Distinct().Stringy(',') , Refine = w.trk?.Select(t=>t.name.Refine()).Distinct().Stringy(',') , Dragstr = w.trk?.Select(t=>t.name.Dragstr().Draglet()).Average().Dragstr() , Gradstr = w.trk?.Select(t=>t.name.Gradstr().Gradlet()).Average().Gradstr() , Flowstr = w.trk?.Select(t=>t.name.Flowstr().Flowlet()).Average().Flowstr() } ).Set(w=>w[0].Set(p=>w.Date=p.Date)).Correct().Altify().Altismooth().Energize() ;
	...
	public partial class trkType
	{
		...
		public static implicit operator Path( trkType track ) => track.Get( t => new Path(t.First.First.time,t.Iterator,Translation.Kind,(Axis.Beat,60)) { Object = t.type , Action = t.name.Action() , Subject = t.link.At(0)?.href.Subject() , Locus = t.name.Locus() , Refine = t.name.Refine() , Dragstr = t.name.Dragstr() , Gradstr = t.name.Gradstr() , Flowstr = t.name.Flowstr() } ).Correct().Altify().Altismooth().Energize() ;
