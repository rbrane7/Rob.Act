...
namespace Rob.Act.Gpx
{
	...
	public partial class trkType
	{
		...
		public static implicit operator Path( trkType track ) => track.Get( t => new Path(t.First.First.time,true,t.Iterator) { Object = t.type , Spec = t.name , Action = t.name.Action() , Subject = t.link.At(0)?.href.Subject() , Locus = t.name.Locus() , Refine = t.name.Refine() } ) ;
	...
	public static class Extension
	{
		...
		internal static string Locus( this string value ) =>value.RightFrom('?').Separate('&',';').Arg("Locus") ;
		internal static string Refine( this string value ) =>value.RightFrom('?').Separate('&',';').Arg("Refine") ;
	}
}
