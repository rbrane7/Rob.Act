...
namespace Rob.Act.Gpx
{
	...
	public partial class trkType
	{
		...
		public static implicit operator Path( trkType track ) => track.Get( t => new Path(t.First.First.time,true,t.Iterator) { Object = t.type , Action = t.name.Action() , Subject = t.link.At(0)?.href.Subject() , Locus = t.name.Locus() , Refine = t.name.Refine() , Dragstr = t.name.Dragstr() , Gradstr = t.name.Gradstr() , Flowstr = t.name.Flowstr() } ).Correct().Altify().Altismooth().Energize() ;
