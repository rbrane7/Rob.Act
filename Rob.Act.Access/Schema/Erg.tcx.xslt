...
<x:stylesheet version="1.0" xmlns:x="http://www.w3.org/1999/XSL/Transform" xmlns:m="urn:schemas-microsoft-com:xslt" exclude-result-prefixes="m tcx #default"
  ...
  <x:template match="tcx:Id">
	...
	<x:if test="not(../tcx:Drag) and $Drag"><Drag><x:value-of select="$Drag"/></Drag></x:if>
	<x:if test="not(../tcx:Action) and $Action"><Action><x:value-of select="$Action"/></Action></x:if>
	<x:if test="not(../tcx:Subject)"><Subject>Rob</Subject></x:if>
	<x:if test="not(../tcx:Locus)"><Locus>Home</Locus></x:if>
	<x:if test="not(../tcx:Refine) and $Refine"><Refine><x:value-of select="$Refine"/></Refine></x:if>
	<x:if test="not(../tcx:Detail) and $Detail"><Detail><x:value-of select="$Detail"/></Detail></x:if>
	<x:if test="not(../tcx:Temp) and $Temp"><Temp><x:value-of select="$Temp"/></Temp></x:if>
	<x:if test="not(../tcx:Pres) and $Pres"><Pres><x:value-of select="$Pres"/></Pres></x:if>
	<x:if test="not(../tcx:Humi) and $Humi"><Humi><x:value-of select="$Humi"/></Humi></x:if>
  </x:template>
