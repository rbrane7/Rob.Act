		...
		public class Skierg
		{
			...
			public Skierg( string data )
			{
				...
				foreach( var line in data.SeparateTrim('\n').Select(l=>l.Trim()) )
				{
					...
					if( time<(accu.Time-TimeSpan.FromTicks(1)).TotalSeconds-atime ) { lap = laps?.FirstOrDefault(l=>l.time>=last.time) ; atime = lap?.time ?? last.time ; adist = last.dist+(lap?.time-last.time??0)*velo ; }
