		...
		public class Skierg
		{
			...
			public Skierg( string data )
			{
				...
				foreach( var line in data.SeparateTrim('\n').Select(l=>l.Trim()) )
				{
					...
					for( var i=ib ; i<=db ; i+=ib )/*interpolation*/{ accu.Time += dt ; accu.Distance = dist ; accu.Work = 2.8*Math.Pow(dist/time,3)*time ; accu.Beat += beat*dt.TotalSeconds/60 ; accu.Effort += power*dt.TotalSeconds ; accu.Drag += (drag.nil()??idrag)*ib/100D ; Data.Add(accu) ; }
				}
			}
