		...
		public class Skierg
		{
			...
			public Skierg( string data )
			{
				...
				foreach( var line in data.SeparateTrim('\n').Select(l=>l.Trim()) )
				{
					...
					for( var i=ib ; i<=db ; i+=ib )/*interpolation*/{ accu.Time += dt ; accu.Distance = dist ; accu.Work = 2.8*Math.Pow(dist/time,3)*time ; accu.Beat += beat*dt.TotalSeconds/60 ; accu.Effort += power ; accu.Drag += (drag.nil()??idrag)*ib ; Data.Add(accu) ; }
				}
			}
			public static implicit operator Path( Skierg work ) => new Path(work.Date,work.Data.Select(p=>new Point(work.Date+p.Time){ Time = p.Time , Dist = p.Distance , Ergy = p.Work , Beat = p.Beat , Bit = p.Bit , Effort = p.Effort , Drag = p.Drag })){ Action = work.Spec , Tag = $"Skierg {work.Data[work.Data.Count-1].get(d=>d.Drag/d.Bit)}" }
