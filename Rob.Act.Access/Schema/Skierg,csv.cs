...
namespace Rob.Act
{
	...
	public static partial class Csv
	{
		public class Skierg
		{
			...
			public static implicit operator Path( Skierg work ) => new Path(work.Date,work.Data.Select(p=>new Point(work.Date+p.Time){ Time = p.Time , Dist = p.Distance , Ergy = p.Work , Beat = p.Beat , Bit = p.Bit , Effort = p.Effort , Drag = p.Drag })){ Action = work.Spec , Tag = $"Skierg {work.Data[work.Data.Count-1].get(d=>d.Drag/d.Bit):0.00} {work.Subject} {work.Locus}" }
