...
namespace Rob.Act
{
	...
	public static partial class Csv
	{
		public class Skierg
		{
			...
			IList<(TimeSpan Time,double Distance,double Beat,uint Bit,double Energy,double Drag)> Data = new List<(TimeSpan Time,double Distance,double Beat,uint Bit,double Energy,double Drag)>() ;
			DateTime Date = DateTime.Now ; string Spec , Subject , Locus , Refine ;
			public Skierg( string data )
			{
				(TimeSpan Time,Quant Distance,Quant Beat,uint Bit,Quant Energy,Quant Drag) accu = (TimeSpan.Zero,0,0,0,0,0) ;
				...
				foreach( var line in data.SeparateTrim('\n').Select(l=>l.Trim()) )
				{
					...
					(uint bit,double time,double dist,uint beat,uint power,uint drag,double pace) = (vals[0].Parse<uint>(0),vals[1].Parse<double>(0),vals[2].Parse<double>(0),vals[7].Parse<uint>(0),vals[4].Parse<uint>(0),vals.At(8).Parse<uint>(0),vals[3].Parse<double>(0)) ;
					...
					for( var i=ib ; i<=db ; i+=ib )/*interpolation*/{ accu.Time += dt ; accu.Distance = dist ; accu.Beat += beat*dt.TotalSeconds/60 ; accu.Energy += power*ds/vq/*dt.TotalSeconds*/ ; accu.Drag += (drag.nil()??idrag)*ib/100D ; Data.Add(accu) ; }
				}
			}
			public static implicit operator Path( Skierg work ) =>
				new Path(work.Date,work.Data.Select(p=>new Point(work.Date+p.Time){ Time = p.Time , Dist = p.Distance , Energy = p.Energy , Beat = p.Beat , Bit = p.Bit , Drag = p.Drag.nil() }))
				{ Action = work.Spec , Tags = $"{Basis.Device.Skierg.Code} {work.Data[work.Data.Count-1].get(d=>d.Drag/d.Bit):0.00} {work.Subject} {work.Locus} {work.Refine}" }
				.Set(p=>{ var l = work.Data[work.Data.Count-1] ; var f = work.Data[0] ; p.Dist = l.Distance-f.Distance ; p.Time = l.Time-f.Time ; p.Energy = l.Energy-f.Energy ; p.Beat = l.Beat-f.Beat ; p.Bit = l.Bit-f.Bit ; p.Drag = l.Drag-f.Drag ; }) ;
