...
namespace Rob.Act
{
	...
	class Program
	{
		...
		{
			Path path = null ; foreach( var part in args.Select(arg=>arg.ReadAllText().Internalize().Set(p=>p.Spec=arg)*(Operation&~Performed(arg))).OrderBy(p=>p.Date) ) path |= part ;
			path.Spec.Get(f=>$"{f.LeftFromLast('.')}.{(Operation&~Performed(f)).Stringy().Replace('|','.').ToLower()}.{f.RightFrom('.',true)}").Set(f=>f.WriteAll((path).Externalize(f.RightFrom('.')))) ;
		}
	}
}
